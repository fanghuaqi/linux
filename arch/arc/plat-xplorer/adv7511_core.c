...
static void adv7511_read_config(struct adv7511 *adv7511)
{
	...
	for (i = 0; i < 16; i++ ) {
		ret = regmap_bulk_read(adv7511->regmap, 16*i, (void *)&adv7511->edid_buf[16*i], 16);
		if(ret) {
			printk(KERN_INFO "read adv7511 config error");
			return;
		}
	}
...
/**
* @brief parse the options of ADV7511's input. Through these options, ADV7511 can work
*	 with different graphical devices, such ARC PGU, GPU, etc..
*
* @param np
* @param config
*
